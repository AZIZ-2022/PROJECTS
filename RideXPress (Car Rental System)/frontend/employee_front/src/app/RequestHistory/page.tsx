...
import { FaCheckCircle, FaTimesCircle, FaUser, FaEnvelope, FaPhone, FaMapMarkerAlt, FaRoute,FaSignOutAlt } from 'react-icons/fa';
...
export default function BookingRequestHistoryPage() {
  const router = useRouter();
  const [bookings, setBookings] = useState<Booking[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [activeMenu, setActiveMenu] = useState('RequestHistory');
  ...
  const fetchBookingHistory = async () => {
    ...
    try {
      ...
      const filteredBookings = res.data.filter((booking: Booking) => booking.status !== 'Pending');
          ...
          <div className="max-w-7xl mx-auto">
            <div className="text-center mb-12">
              ...
              <p className="text-gray-400 max-w-lg mx-auto">
                All Accepted and Canceled ride requests
              </p>
            </div>
            ...
            {!loading && !error && (
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {bookings.map((booking) => (
                  <div 
                    key={booking.id} 
                    className={`bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border shadow-xl transition-all hover:shadow-2xl ${
                      booking.status === 'Accepted' ? 'border-green-500/30' : 'border-red-500/30'
                    }`}
                  >
                    <div className="space-y-4">
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-gray-400 flex items-center gap-1 mt-1">
                            <FaPhone className="text-sm" /> {booking.phoneNumber}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-gray-300">{booking.pickupLocation}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <FaMapMarkerAlt className={`text-red-400 flex-shrink-0`} />
                          <p className="text-gray-300">{booking.destinationLocation}</p>
                        </div>
                      </div>
                      ...
                      {/* Map Links */}
                      {booking.pickupMapUrl && booking.destinationMapUrl && (
                        <div className="flex gap-3 pt-4">
                          ...
                          <a
                            ...
                            className="flex items-center gap-1 text-sm text-blue-400 hover:text-blue-300 transition-colors"
                          >
                            <FaMapMarkerAlt size={12} /> Destination
                          </a>
                          {booking.routeMapUrl && (
                            <a
